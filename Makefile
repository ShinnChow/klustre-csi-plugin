...
.PHONY: deps build fmt fmt-fix lint test image image-multiarch clean check-manifest-version check-cargo-version tag tag-push tag-delete tag-repush release
...
check-cargo-version:
	...
	if [ -z "$$CARGO_VERSION" ]; then \
	  echo "ERROR: Could not determine version from Cargo.toml" >&2; exit 1; \
	fi; \
	if [ "v$$CARGO_VERSION" != "$(VERSION)" ]; then \
	  echo "ERROR: Version in Cargo.toml ($$CARGO_VERSION) does not match VERSION ($(VERSION))." >&2; \
	  echo "Update Cargo.toml [package].version before tagging a release." >&2; \
	  exit 1; \
	fi
...
tag:
	...
	@$(MAKE) check-cargo-version VERSION=$(VERSION)
